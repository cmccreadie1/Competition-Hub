const APP_VERSION = "v7.4.0"; // Version update for Embedded Manual
    ...
    // Rebalance remainder to minimize relative percentage displacement
            ...
            html += `
                <tr style="border-bottom: 1px solid rgba(255, 255, 255, 0.15); background: rgba(15, 23, 42, 0.1); font-weight: 600; height: 54px;">
                    ...
                    <td style="padding: 12px 8px; font-family:monospace; font-size:16px; background: rgba(59, 130, 246, 0.02); border-left: 1px solid rgba(255,255,255,0.05); border-right: 1px solid rgba(255,255,255,0.05); letter-spacing: 0.5px;">
                        <span style="display:inline-block; width:55px; color:var(--accent); font-weight:900; background:rgba(59, 130, 246, 0.15); border: 1px solid #3b82f6; border-radius: 4px; padding:2px 0; text-align:center;">${row.d1.pts}</span>
                    ...
                    <td style="padding: 12px 8px; font-family:monospace; font-size:16px; background: rgba(234, 179, 8, 0.04); letter-spacing: 0.5px;">
                      <span style="display:inline-block; width:55px; color:var(--accent); font-weight:900; font-size:17px; background:rgba(234, 179, 8, 0.15); border: 1px solid #3b82f6; border-radius: 4px; padding:2px 0; text-align:center;">${row.comb.pts}</span>
...
function calculateAndRenderTeamLeaderboard(containerId) {
    ...
    <div style="background: rgba(30, 41, 59, 0.99); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.2);">
        ...
        if (teamList.length === 0) {
            html += `<tr><td colspan="5" style="padding:40px; color:#64748b; font-weight:800; font-size:16px; text-align:center;">No registered teams found with scored data logs.</td></tr>`;
        } else {
...
// BIGGEST FISH TOP 10 LEADERBOARD MATH ENGINE
function calculateAndRenderBiggestFishLeaderboard(containerId) {
    const container = document.getElementById(containerId);
    if (!container) return;
                ...
                <table style="width: 100%; border-collapse: collapse; text-align: center; font-size: 15px; color: #ffffff; white-space: nowrap;">
                    <thead>
                        <tr style="border-bottom: 2px solid rgba(255,255,255,0.15); color: #94a3b8; font-weight: 800; font-size: 12px; background: rgba(15, 23, 42, 0.3);">
                            ...
                            <th style="padding: 10px 12px; text-align: right; width: 90px;">SIZE (CM)</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${tableRows}
